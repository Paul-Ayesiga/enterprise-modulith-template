...
import type { BlocklistEntry } from "../lib/gateway";

/**
 * The edge deny-list: what is blocked right now, block another source, lift one. Every entry
 * refuses matching clients before auth, quotas, or routing — the cheapest possible "no".
 */
export function BlocklistPanel({ entries, error }: { entries: BlocklistEntry[]; error: string | null }) {
            ...
            <ul className="blocklist" style={{ listStyle: "none", margin: "0 0 0.75rem", padding: 0 }}>
              {entries.map((entry) => (
                ...
                >
                  ...
                  <span className="badge badge--open">{entry.source}</span>
