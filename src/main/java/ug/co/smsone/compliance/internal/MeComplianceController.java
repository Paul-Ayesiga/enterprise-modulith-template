...
class MeComplianceController {
    ...
    List<ResourceObject> consents(CurrentUser user) {
        return compliance.consentHistory(user.subject()).stream()
                .map(c -> new ResourceObject(user.subject(), "consent",
    ...
    ResourceObject record(@RequestBody ConsentRequest request, CurrentUser user) {
        var saved = compliance.recordConsent(user.subject(), request.purpose(), request.granted(),
                request.source() == null ? "api" : request.source());
        return new ResourceObject(user.subject(), "consent",
    ...
    Map<String, Object> export(CurrentUser user) {
        return compliance.dataExport(user.subject());
    }
    ...
    ResourceObject requestErasure(CurrentUser user) {
        ErasureRequest request = compliance.requestErasure(user.subject(), user.accountableSubject());
