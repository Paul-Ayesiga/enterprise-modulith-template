...
-- role_id is a real foreign key: org_group and org_role are the same module AND the same tier
-- (AGENTS §1's two FK clauses). It went unenforced when this was written, sitting next to
-- membership.role_id which was enforced — an inconsistency with no reason behind it.
--
-- org_id IS NOT, and the story is worth keeping because it reversed twice. It could not be a foreign
-- key at all until organization.id became the tenant key (V11) — it held a Keycloak identifier, and
-- there is nothing to point a foreign key at in Keycloak. V30 then made it one. V53 cut it again,
-- because `organization` is platform-tier and a group is the tenant's; the split moves that cut here,
-- since `organization` is not on this sequence's search_path and after Phase 7 not in this database.
-- Same soft-ref reasoning as membership.org_id — see the tenant half of V11's header.
