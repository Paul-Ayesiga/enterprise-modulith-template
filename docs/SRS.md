...
| FR-SCH-6 | honor per-org RETENTION overrides for the org-scoped logs (`webhook-delivery`, `exchange-job`): each retention job purges non-overridden orgs at the platform default, then each overridden org at its OWN cutoff (the shared `RetentionOverrides` port + `RetentionPurges`); the platform sets them at `/api/v1/admin/orgs/{id}/retention/{scope}` (platform-admin, audited). Notification deliveries are recipient-scoped and excluded by design | `main:scheduler/internal/{OrgRetentionOverride,RetentionOverridesImpl,AdminRetentionController}`, `main:shared/retention/*`, `V39` | `test:shared/retention/RetentionPurgesTest`; `test:exchange/internal/ExchangeRetentionOverrideTest`; `test:scheduler/internal/RetentionOverrideApiTest` |
...
The reference spec is `docs/plans/reusable-data-exchange-platform-guidelines.md`; the module is its
...
| FR-BIL-6 | let the tenant MANAGE payment methods on its own surface (`/api/v1/orgs/{orgId}/billing/payment-methods`): list (`org:read`), add / set-default / remove (`org:update`), each proxied to a Kill Bill payment PLUGIN so raw card data never reaches this API — `pluginProperties` carries only a hosted-page/plugin token; the surface stays reachable while a subscription is paused (the pay-to-recover hatch) | `main:billing/internal/{OrgBillingController,BillingService,KillBillGateway}` | `test:billing/internal/BillingApiTest.paymentMethodsAreManagedOnTheOrgSurfaceAndRequireOrgUpdate`; `test:billing/internal/KillBillIntegrationTest.paymentMethodsRoundTripAgainstRealKillBill` |
...
### 3.23 Devices and organization security policies (FR-ACC)
...
### 3.24 Integration hub (FR-INT)

The system SHALL:

| ID | Requirement | Where | Verified by |
|---|---|---|---|
| FR-INT-1 | hold provider configs (SMS/email/payment gateway) at platform-default and org-override scope, one live per (scope, kind), and resolve deterministically through the `Integrations` port — org override, else platform default, else empty | `main:integration/{Integrations}.java`, `main:integration/internal/{IntegrationsImpl,IntegrationRepository}` | `test:integration/internal/IntegrationHubTest.anOrgOverrideWinsOverThePlatformDefaultAndSecretsAreEncrypted` |
...
| FR-SUP-5 | answer the cross-tenant queue (`GET /api/v1/admin/tickets`) from `platform.ticket_index` — one platform-side keyset page whose cost is a function of the page, not of the tenant count — maintained from every tenant-tier ticket write inside that write's own transaction, and (for a tenant on another database) in its own transaction on primary, ordered so that only the benign drift survives: a queue row may outlive its ticket, never the reverse | `main:support/internal/{TicketIndex,TicketFanOut,SupportService,SlaEscalationJob}`, `V61` | `test:support/internal/TicketIndexTest.aTicketOpenedInThePoolAndOneOpenedInASiloBothReachTheOperatorQueue`, `.theQueuePageComesFromTheIndexAndNotFromTheTenantSchemas`; `test:support/internal/RemoteTenantTicketIndexTest.aTicketOpenedByARemoteTenantLivesOnTheRemoteAndItsQueueRowOnPrimary` |
| FR-SUP-6 | keep that projection honest without a foreign key it may not have: `TicketIndexReconciler` fans out over every tenant home nightly (PT30M lease, 25-minute run deadline, resumable per home and per arm) and repairs both directions — a live ticket the index never learned about, and a queue row for a ticket the home no longer has. It **is** the delete path, not a backstop for one: nothing in the application soft-deletes a ticket, so every disappearance is raw SQL that cannot know the projection exists. The sweep arm never reasons from absence — each page of index rows is checked against the tenant that owns it, so a deadline-cut run is a shorter run and not a mass deletion | `main:support/internal/TicketIndexReconciler` (ADR 0010 §8 Q2) | `test:support/internal/TicketIndexTest.theQueueKeepsShowingAHardDeletedTicketUntilTheReconcilerRuns`, `.theReconcilerBacksFillsATicketThatWasWrittenAroundTheIndex`; `test:support/internal/RemoteTenantTicketIndexTest.theReconcilerRemovesAQueueRowForATicketDeletedOnTheRemote` |
| FR-SUP-7 | keep the index an operator convenience and never an authority: a tenant's own ticket reads go to its own schema, the operator's single-ticket routes read the tenant's own row, and `TicketFanOut.onTicketsHome` uses the index only as a routing HINT that falls back to the per-home probe when it is absent or wrong | `main:support/internal/{TicketFanOut,OrgTicketController,AdminTicketController}` | `test:support/internal/TicketIndexTest.aTenantReadsItsOwnTicketsFromItsOwnSchemaAndNeverFromTheIndex`; `test:support/internal/RemoteTenantTicketIndexTest.theOperatorsSingleTicketRouteStillReadsTheRemoteTenantsOwnRow` |
...
| POST | `/api/v1/orgs/{orgId}/geo/stamps` | `geo:capture` | 201 (the stored stamp) | 422 (capture off / coordinate out of range / too coarse / stale / source not allowed) |
| GET | `/api/v1/orgs/{orgId}/geo/stamps` | `geo:read` | 200 (paged; `subjectType`+`subjectId` or `bbox` filter; coarsened unless `geo:read_precise`) | 422 bad `bbox`/cursor |
| GET/PUT | `/api/v1/orgs/{orgId}/geo/policies/{subjectType}` | `geo:policy:manage` | 200 (the policy; default OFF) | 422 bad mode |
...
| NFR-TEN-9 | Every object-storage key SHALL name its owner: an organization's bytes under one of that organization's **declared prefixes** (`doc/o/<orgId>/`, `exch/o/<orgId>/` — `shared.document.OrgObjectPrefixes`), a person's under a person segment (`doc/u/…`, `u/…`, `avatar/p/…`) that SHALL NOT sit under any organization's prefix. An extraction copies objects **by prefix** (ADR 0010 §6 item 7) and `uq_document_storage_key` is per-schema since the tier split, so the prefix is both the extraction manifest and the only remaining cross-tenant collision guard. Carrying the `o/<orgId>/` segment under an **undeclared root** SHALL be refused too — the bundle lists prefixes, so such a key is one no extraction would visit | `test:shared/document/StorageKeyNamespaceTest` (the `NewDocument` guard, both directions, the undeclared-root case, plus the enumeration of every writer of object storage); `test:document/internal/DocumentExtractionBoundaryTest.anOrganizationsDocumentIsKeyedUnderThatOrganizationAndFiledInItsSchema` |
| NFR-TEN-10 | A **personal** document (`document.org_id IS NULL`) SHALL be written to and read from the platform copy of the split table regardless of which organization the caller's token names, so it neither travels with a `pg_dump -n t_<hex>` extraction nor is stranded by a promotion nor disappears when its owner joins a second organization | `test:document/internal/DocumentExtractionBoundaryTest.aPersonalDocumentUploadedByASingleOrgMemberIsFiledOutsideThatTenantsSchema`, `.aPersonalDocumentIsStillReachableAfterItsOwnerStopsResolvingToOneOrganization` |
| NFR-TEN-11 | An organization's object-store extraction SHALL carry every object under that organization's declared prefixes and no other object, SHALL restore each one byte-identical **with its stored content type**, and SHALL refuse rather than under-copy: it runs pinned to the tenant, checks the tenant's own recorded object keys (`document.storage_key`, `exchange_job.source_key`/`result_key`/`error_report_key`) against those prefixes before any byte moves, pages on the store's own truncation flag rather than on page size, and refuses any bundle object that is not this organization's | `test:compliance/internal/TenantObjectExtractionTest` (all ten cases, against real SeaweedFS); `test:files/FileStorageIntegrationTest.listPagesByKeysetAndReportsTruncationFromTheStoreRatherThanFromThePageSize`, `.openCarriesTheContentTypeAndLengthTheStoreHolds`, `.writeRoundTripsBothSidesOfTheMultipartThreshold` |
| NFR-TEN-12 | An extraction bundle SHALL account for **every one** of ADR 0010 §6's eleven items — a manifest missing one SHALL NOT be constructible — and SHALL be **structurally incapable** of carrying `event_publication`, `event_inbox`, `idempotency_key`, `shedlock`, `queue_signal` or `tenant_freeze`. It SHALL refuse rather than produce a bundle that looks whole: a platform-tier table with no recorded disposition, a live legal hold naming the org, an undrained `notification_delivery` or an in-flight `webhook_delivery`/`exchange_job`, a subscription naming a plan the catalogue does not hold, an empty seeded catalogue, a CUTOVER on an unfrozen tenant, a tenant-pinned thread, or an open transaction. **Each platform table's disposition SHALL be recorded exactly once**: `TenantBundlePlan` is the judgement, and any second record of it (the promotion-side mirror the self-containment test reads) SHALL fail the build if it disagrees in either direction | `test:compliance/internal/TenantBundleTest` (including the real restore into a second database), `test:compliance/internal/TenantBundlePlanTest`, `test:compliance/internal/ExtractionVerdictAgreementTest`, `test:compliance/internal/PersonProjectionGuardTest` |
| NFR-TEN-13 | An extracted deployment SHALL inherit the platform's catalogue **with its identifiers** (`plan`, `plan_entitlement`, `sla_policy`, `translation`, `setting`, `feature_flag`). An installation whose `org_subscription.plan_id` names a plan `platform.plan` does not hold SHALL **fail startup** naming the plans when the schema at fault is one it cannot serve anybody without — `tenant_pool`, or the single silo of an installation whose `platform.tenant_placement` holds exactly one tenant, which is the extracted deployment — and SHALL otherwise **log a warning naming the organization, the plans and the query** rather than convert one tenant's silent downgrade into every pod's refusal to start. A silo whose schema the registry names but the database does not hold SHALL be reported by the same check and SHALL NOT be thrown out of the `ApplicationReadyEvent` listener | `test:subscription/internal/PlanCatalogGuardTest` (all four cases), `test:compliance/internal/TenantBundleTest.theCatalogueTravelsWholeAndASubscriptionNamingAPlanItDoesNotHoldRefusesTheBundle` |
| NFR-TEN-14 | Extracting an organization SHALL revoke its API keys and reserve their prefixes **permanently**, and every mint SHALL consult those reservations — a soft-deleted key releases its prefix from `uq_api_key_prefix_live`, and re-minting a departed tenant's prefix makes a stale credential resolve against another tenant's row on the pre-tenant lookup | `test:compliance/internal/TenantBundleTest.aCutoverRevokesTheOrgsKeysAndReservesTheirPrefixesWhileARehearsalRevokesNothing`, `test:ArchitectureTests.onlyTheReservationAwareMintCallsApiKeyHashingMint` |
...
| FR-FIL-1..8, 9 ✗, 10..12, 13 ✗ | `main:files/internal/{FileController,S3StorageProvider,S3ClientConfig,StorageProperties,BucketBootstrap}.java`, `main:files/{FileStorageProvider,FileStorageException,FileNotFoundException,ObjectPage,StoredObject}.java` | `test:files/internal/FileApiTest` (10), `test:files/FileStorageIntegrationTest` (8), `test:files/ResilienceSmokeTest` |
...
| FR-SUP-1..7 | `main:support/{SupportDesk,TicketEscalated}.java`, `main:support/internal/{SupportService,SupportDeskImpl,OrgTicketController,AdminTicketController,AdminSlaController,Ticket,TicketRepository,TicketMessage,TicketFanOut,TicketIndex,TicketIndexReconciler,SlaEscalationJob,SlaPolicySeeder,OrgSlaOverride,SupportNotifier,TicketResources}.java`, **`V36__support.sql`**, `V38__org_sla_override.sql`, **`V61__ticket_index.sql`** | `test:support/internal/{SupportFlowTest,SlaOverrideTest,SupportDeskFanOutTest,TicketIndexTest,RemoteTenantTicketIndexTest}` — the last two are the V61 projection and its reconciler, the second of them across two real Postgres containers (ADR 0011) |
...
| NFR-TEN-1..14 | see §5.2; `main:shared/document/{OrgObjectPrefixes,NewDocument}.java`, `main:compliance/internal/TenantObjectExtractionService.java` for 9 and 11; `main:compliance/internal/{TenantBundler,TenantBundlePlan,PersonProjector,BundleScriptWriter}.java`, `main:apikeys/ApiKeyReminting.java`, `main:subscription/internal/PlanCatalogGuard.java` for 12–14 | `test:organization/internal/{OrgRbacApiTest,OrgRbacAuthorityTest}`, `test:webhooks/internal/WebhookApiTest`, `test:files/internal/FileApiTest`, `test:shared/ratelimit/RateLimitIntegrationTest`, `test:shared/idempotency/IdempotencyIntegrationTest`, `test:shared/document/StorageKeyNamespaceTest`, `test:document/internal/DocumentExtractionBoundaryTest`, `test:compliance/internal/TenantObjectExtractionTest`, `test:files/FileStorageIntegrationTest`, `test:compliance/internal/{TenantBundleTest,TenantBundlePlanTest,PersonProjectionGuardTest}`, `test:subscription/internal/PlanCatalogGuardTest` |
