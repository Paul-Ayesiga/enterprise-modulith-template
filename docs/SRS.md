...
| NFR-TEN-9 | Every object-storage key SHALL name its owner: an organization's bytes under an `o/<orgId>/` segment (`doc/o/…`, `exch/o/…`), a person's under a person segment (`doc/u/…`, `u/…`, `avatar/p/…`) that SHALL NOT sit under any organization's prefix. An extraction copies objects **by prefix** (ADR 0010 §6 item 7) and `uq_document_storage_key` is per-schema since the tier split, so the segment is both the extraction manifest and the only remaining cross-tenant collision guard | `test:shared/document/StorageKeyNamespaceTest` (the `NewDocument` guard, both directions, plus the enumeration of every writer of object storage); `test:document/internal/DocumentExtractionBoundaryTest.anOrganizationsDocumentIsKeyedUnderThatOrganizationAndFiledInItsSchema` |
...
| NFR-TEN-1..10 | see §5.2 | `test:organization/internal/{OrgRbacApiTest,OrgRbacAuthorityTest}`, `test:webhooks/internal/WebhookApiTest`, `test:files/internal/FileApiTest`, `test:shared/ratelimit/RateLimitIntegrationTest`, `test:shared/idempotency/IdempotencyIntegrationTest`, `test:shared/document/StorageKeyNamespaceTest`, `test:document/internal/DocumentExtractionBoundaryTest` |
