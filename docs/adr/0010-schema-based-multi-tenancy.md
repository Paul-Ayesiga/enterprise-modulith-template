...
> *Re-measured 2026-08-08 against 200 real silos (§8 Q1). **The conclusion of this paragraph stands; every figure in it moved.** These are the UNIFORM counts — every table in every schema — and the hybrid's tenant tier is 28 tables, so at 5,000 schemas it is ~765,000 `pg_class` rows, ~3.89M `pg_attribute` rows, ~2.65 GB of catalog, ~4.35 GB of empty files and ~1.56M metadata rows. Locks are **7.000** per branch, not 2.004 (the planner locks a table and all six of `ticket`'s indexes), and the lock table is not capped at `max_locks_per_transaction × max_connections` — one transaction reached 14,751 entries — so the UNION wall is ~2,100 branches, not ~3,200. Uniform schema-per-tenant is still not shippable, at roughly 40% of the magnitude argued here.*
...
```
platform.queue_signal(queue text, org_id uuid, due_at timestamptz, lease uuid,
                      primary key (queue, org_id))
```

Enqueue upserts one signal row per **batch**, not per row. Claim becomes two steps: claim a *tenant* from `queue_signal` with the `FOR UPDATE SKIP LOCKED` idiom already in `WebhookDeliveryQueue`, then run today's claim query with `and org_id = ?` appended and the `search_path` pinned. Release recomputes `due_at` from the tenant's remaining rows — PENDING `next_attempt_at` **plus** PROCESSING `locked_at + staleLock`, do not lose the stale-reclaim arm — and deletes the row when nothing is left. A stale signal is harmless: a worker that finds nothing deletes it.

Two details of that first step are load-bearing, and both were learned the hard way.

**The candidate must be a MATERIALIZED CTE, not an `UPDATE … FROM (select … limit 1 …)`.** Lifted verbatim, the idiom does not survive being the *whole* claim: as a plain join input the candidate may be rescanned once per outer row, `LockRows` skips the row the same command already updated, and each rescan therefore returns the *next* due tenant. One claim then leases every due tenant and returns one — the others parked behind a lease no worker holds for a full stale-lock window, which is this section's own starvation reintroduced inside the mechanism meant to end it. Whether it happens is a join-order choice, so it comes and goes with the statistics; it cost three fairness tests that failed in one suite run and passed in the next against identical code. `as materialized` makes the candidate a tuplestore evaluated exactly once, so a claim can only ever lease one tenant whatever plan the database picks.

**The release is fenced on `lease`, a uuid, and not on `due_at`.** A worker leaves with a tenant and comes back a whole batch later on another connection, so its release has to prove it is still the holder — otherwise it overwrites the `due_at` of whoever claimed the tenant after it, or buries work enqueued while it was away. Fencing on "`due_at` is still what the claim stamped" looks cheaper and is not: it makes identity a timestamp comparison across a round trip (silent total loss of fairness the first time anything rounds it), and it forces the lease expiry to be minted wherever the token is minted — from the JVM's clock, while `due_at <= now()` reads the database's. A uuid separates the two questions the row answers: `due_at` says WHEN the tenant is claimable, always on the database's clock; `lease` says WHO holds it. `raise` clears the token, so an enqueue that lands mid-batch always beats the release that follows it.
...
Schema names derive from `organization.id`: `"t_" + uuid.toString().replace("-","")` — 34 chars, well under NAMEDATALEN 63, deterministic, **zero database reads to route**.

> *Phase 5 amendment (2026-08-08). The NAME still costs zero reads and that is the property this sentence was protecting — nothing in the binary ever holds a silo name, which is why promotion is a row change and not a deployment. What cannot be derived is **whether this tenant has been promoted yet**, and that is a row. `shared.tenancy.TenantRoutes` reads `platform.tenant_placement` and memoizes the answer per organization for `app.tenancy.route-ttl` (30 s), so the borrow path costs a map lookup and the registry is read once per tenant per interval per pod. It reads through the RAW pool, never the routing DataSource, or resolving a route would recurse; and `TenantRoutingDataSource` resolves the path BEFORE it takes its connection, so a cache miss is a sequential extra borrow rather than a second connection held on top of the first.*
> *The memo's TTL is a correctness parameter, not a cache size: the flip drops it in the promoting process only, so `TenantPromoter` holds the freeze one full TTL after the flip before deleting the source rows or unfreezing — §6 hop 0→1's step list gains that wait. A read that fails is rethrown and the borrow fails; falling back to `tenant_pool` would be §1's worst failure, and it buys nothing while the registry is in the same database as the statement that was about to run. Phase 7 changes that last sentence and nothing else here.* Never the alias: `OrganizationService.rename` exists and `uq_organization_alias_live` is partial, so an alias is both mutable and reusable. Validate against `^t_[0-9a-f]{32}$` before interpolation; that regex is the only thing between a schema name and SQL injection.
...
| `org-permissions` (`PermissionResolver:37`) | `#organizationId + ':' + #personId` | TENANT. **The organization stays in the key — see below. This question is closed.** |
| `org-entitlements` (`EntitlementResolver:32`) | `#orgId.toString()` | TENANT; hand-rolled prefix dropped, replaced by `TenantCacheKeys.forThisTenant(#orgId)`, which *throws* unless the axis is the org being asked about. |
...
#### 3.5.1 The `org-permissions` organization stays in the key. Permanently — this is settled, not deferred

The Phase 3 table above originally read "drop the hand-rolled prefix" for both TENANT caches, on the observation that once the tenant is a schema the prefix is the isolation and the key need not repeat the org. That argument was refused in Phase 3 on the ground that pooled tenants share `tenant_pool`, and the refusal was deferred to Phase 5 with "remove it together with the probe". **Phase 5 has now run the argument to the end and the answer is that it stays, for a reason that never expires.**

The prefix `TwoLevelCache` writes comes from the **axis**; the `#organizationId` in this key comes from the **argument**. They agree only when the axis names the very organization being asked about. `org-entitlements` can *require* that agreement — `TenantCacheKeys.forThisTenant` throws when it does not — because it holds one entry per tenant. `org-permissions` cannot, because its callers legitimately ask about organizations they are not pinned to.

And §3.4 makes that permanent rather than transitional: **the background fan-out is one axis per HOME, not one per tenant** — that is the whole point of "bounded by the silo ceiling, not by tenant count", and §1 measures why per-tenant iteration is not shippable into this database. `TenantHome.pool()` is that axis for the shared schema: a uuid naming no organization, standing in for every unpromoted tenant there is. A sweep over the pooled home therefore resolves permissions for many different organizations under one cache prefix — `ExchangeScheduleFiringJob` does it today, per due schedule, inside a transaction where `OrgAuthorizationImpl` deliberately does not re-pin. Drop `#organizationId` and a person who belongs to two pooled organizations gets whichever permission set was resolved first. That is a cross-tenant authorization bypass and it is the AGENTS §5.5 invariant, so:

> **The organization stays inside `org-permissions`' key for as long as `tenant_pool` exists — which, per §1 and §4.3, is for as long as this product has customers who have not been promoted. Do not re-open this.** The price is a longer key and coarse eviction (every eviction of this cache is a `clear()`, because an entry written under a home's axis is unreachable by any key that names the organization). `TenantPromotionEvictionTest.theOrganizationInTheKeyIsWhatSeparatesTwoTenantsUnderOneHomesAxis` fails without it.

#### 3.5.2 What a promotion evicts, and what it deliberately does not

Promotion is the one event that invalidates a cached answer **without changing any row**: the tenant's rows are copied to a new schema and the placement flips, so a cache holding a value computed against the old schema keeps serving it — and a cache HIT runs no query, so the routing DataSource, `MappedSchemaValidator` and "that table is not in this schema" never get a chance to notice. One of those values is an authorization decision.

`shared.cache.TenantPromotionCaches.evictAfterPlacementFlip(orgId)` is the whole step, called **after the flip has committed and before the freeze is lifted** (evicting earlier lets a reader repopulate from the old schema; evicting later serves the old answers to live traffic). It classifies every declared cache, and `TenantPromotionEvictionTest` fails the build when a cache is declared in `CacheRegistry` without a verdict here — the same enumeration shape as `CacheDeclarationTest`.

| cache | at the flip | why |
|---|---|---|
| `org-permissions` | **cleared, every tenant** | `membership`, `org_role`, `role_permission` are tenant-tier and move. Keys are per-member under a prefix that may be a home's axis, so no keyed evict is complete — §3.5.1. |
| `org-entitlements` | **evicted, this tenant only** | `org_subscription` is tenant-tier and moves. One entry per tenant under `TenantCacheKeys.wholeTenant()`, and its key helper guarantees only one prefix can hold it. |
| `person-by-subject` | untouched | `person` + `external_identity` are platform-tier; a person does not move when one of their orgs does, and `person.id` is stable across every hop in §6. |
| `org-by-external-id` | untouched | See below. |
| `setting-values`, `feature-flags`, `translation-bundles` | untouched | Platform-tier catalogs with no `org_id`. The per-org flag question over `feature_flag_org_override` (tenant-tier, and it *does* move) is deliberately uncached, which is the only reason `feature-flags` can say this. |

**Correction to §6 hop 0→1, which listed "evict `OrgResolutionCache`" as a required step: it is not, and doing it is a cost with no correctness behind it.** That cache holds `(provider, issuer, external_org_id) → organization.id`, read from `platform.external_organization` — the router's *input*, not the router's answer. Promotion changes where that id routes to; it cannot change the id, and the silo is named after it. There is no cache of the placement to go stale: `TenantSchemas.searchPathFor` re-derives the home on every connection borrow. Evicting it anyway costs the whole installation one indexed read per authenticated request on every promotion. The invariance is a property of the **value**: if this cache ever holds a schema name or a datasource, it becomes routing state and its verdict flips.

One in-process memo is not a Spring cache and is evicted by the same call: `TenantSchemaFloor` remembers "at or above the floor" **forever**, and that being one-way is only true while a tenant stays in one schema. The sanctioned sequence migrates the new home to head before flipping, so the memo stays true — `TenantSchemaFloor.forget(orgId)` is what makes that a property of the data rather than an assumption baked into the memo.
...
> *Re-measured 2026-08-08 (§8 Q1). Planning is **0.024–0.044 ms per branch on a warm backend**, not 0.5–0.66; the quoted figures are cold-backend first executions and they amortize both per backend and, under a server-side prepared statement, per plan. Locks are **7.000** per branch, not 2.004. The 200-branch query text is 67,627 bytes, well inside pgjdbc's 5 MiB statement cache, so it is **not** defeated at the ceiling. And the replacement below is not what shipped: `TicketFanOut` merges one query per home in Java, which holds 9 locks whatever the fleet size but costs **279 ms per page at 200 homes**.*
...
> *SHIPPED as V61 — see §8 Q2's resolution, which also records the three places the sketch above turned out to be wrong (the column list is short of what the wire contract renders, the feed is the tenant-tier write path rather than an event listener, and the reconciler is the delete path rather than a backstop). `TicketFanOut.queue` is one keyset statement against the index; `onTicketsHome` uses it as a routing HINT and still falls back to the per-home probe, because a projection allowed to 404 a live ticket has stopped being an operator convenience.*
...
> *The index exists (V61) and this job still loops, deliberately — see §8 Q2's resolution. A listing can be served from a projection; a breach sweep cannot, because `lockBreached`'s `PESSIMISTIC_WRITE`/`SKIP LOCKED` is what stops two instances escalating one ticket twice, and no projection can lock a row in another schema — let alone in another database. Reading candidates from the index still re-locks each one in its home, so the swap only pays once the breach count is far below the home count, which nobody has measured.*
...
> *Re-measured 2026-08-08 (§8 Q1): a real silo is **126 relations (28 tables + 98 indexes) and 912 KB**, because the tenant tier is 28 tables and not 55 — 239/3.2 MB is the uniform number. At 200 silos: ~31,000 relations, **178 MB** of empty files, **115 MB** of added catalog. The verdict is unchanged; the headroom is 2.6× larger than stated.*
...
> *Re-measured 2026-08-08 (§8 Q1) at 200 schemas, on a **seeded** silo: 2.16 s cold, **0.29–0.30 s warm**, 1.04 MB; `--schema-only` 0.22 s / 48 KB. The same cold/warm split as §5.1 — the degradation with fleet size is real but far weaker than 2.9 s suggests.*
...
> *Re-measured 2026-08-08 (§8 Q1): **30** schemas per transaction, failing on the 31st with exactly that error. Per-schema lock count is close (487, not 439); the capacity figure was wrong — one transaction reaches 14,751 lock entries at the 64 × 100 default, not 6,400. The rule stands and is conservative by 2×.*
...
Create the schema, run the tenant migration set, copy the org's rows with `CREATE TABLE AS SELECT … WHERE org_id = ?` (measured: 1.72 s for 150k rows across the 8 largest tables, including the `ticket_message` parent join), verify, flip `platform.tenant_placement`, then — still inside the freeze — call `TenantPromotionCaches.evictAfterPlacementFlip(orgId)`. (An earlier draft named `OrgResolutionCache` as the cache to evict here; §3.5.2 corrects that and gives the full set. The eviction that matters is `org-permissions`, which holds authorization decisions computed against the old schema; the resolution cache holds an `organization.id` that promotion cannot change.) The freeze window is an org-scoped `maintenance_window` row in `RESTRICT` mode — **but that gates HTTP org paths only**; the notification worker, webhook dispatcher, exchange runners, `OutboxResubmissionJob` and retention purges all write tenant rows outside any request and must be paused for that org too (via `platform.queue_signal` and a job-side placement check).
...
7. The org's bytes from SeaweedFS under `doc/o/<orgId>/` and `exch/o/<orgId>/`. **Those two prefixes are the complete set, verified** (§8 unknown 1, now settled): every org-scoped key in the application carries an `o/<orgId>/` segment and `NewDocument` refuses one that does not. Everything else in the object store is person-scoped and stays — `doc/u/<personId>/`, `u/<personId>/` (raw files, which have no `document` row at all) and `avatar/p/<personId>/`. Copying by prefix is exactly why the personal namespaces must never borrow an `o/` segment: the bundle takes what it finds under the prefix, whatever the row says.
   **Shipped as `compliance.internal.TenantObjectExtractionService`** — `extract(orgId, sink)` streams every object under `OrgObjectPrefixes.forOrg(orgId)` (the single declaration both the extraction and `NewDocument`'s guard read), `restore(orgId, source)` writes a bundle back and refuses per object any key that is not this organization's. Three things about it are load-bearing and none is obvious. The **prefix list is the argument list**, so a key naming its org under an undeclared root — `report/o/<orgId>/…` — is a byte no extraction visits; `NewDocument` therefore checks the whole prefix and not just the segment, and a new namespace is one root added to `OrgObjectPrefixes`. The **database is the detector, not the source**: before a byte moves, the extraction reads every object key the tenant's own schema records (`document.storage_key` plus `exchange_job`'s three) and refuses if any falls outside those prefixes — the alternative is a bundle short by an unknown number of objects that says nothing, and the far side finds out as a download that 404s. And the **listing terminates on the store's `isTruncated`**, never on a short page: S3 permits fewer keys than asked while more remain, so page-size inference is how a bundle ends up short by exactly one page. Promotion (hop 0→1) copies **no** bytes — a key names the organization, not the schema — so the object store moves only at hop 2→3.
...
**Shipped as `compliance.internal.TenantBundler`**, which produces all eleven and a manifest that accounts for each — and where it lives is the first decision worth recording. `compliance.internal`, beside the two artifacts it is a sibling of: a bundler in `shared.tenancy` could not reach `TenantExtractionService` (package-private) and could only invert it behind a port, which would put the interface in the OWNER's world rather than the consumer's and invert AGENTS §2.3 for nothing. It reads many modules' tables through `JdbcTemplate` and imports no module's internals; its one Java dependency is `apikeys.ApiKeyReminting`, a port in that module's API package, because item 8 must revoke through the entity's `@SQLDelete` and reaching into `apikeys.internal` is what `ModularityTests` forbids.

Four things about it are load-bearing and none is obvious. **Item 6 is a property of the type, not a rule someone remembers**: a `BundleDisposition` that does not carry rows has no read SQL anywhere in the package and `PlatformTable.read()` throws for one, so the bundle is *incapable* of carrying `shedlock` — and the set is derived from the catalogue, which is how it holds six tables rather than the four this section names (`queue_signal` V56 and `tenant_freeze` V58 both landed later, both carrying exactly the hazard described here; a hand list would have shipped four). **A platform table with no recorded disposition fails the bundle at plan time, naming itself**, because on the platform side a forgotten table is invisible in both directions and has no later detector. **Item 4's real symptom is a third one this document does not state**: an extracted deployment does not boot with an empty `plan` table — `PlanSeeder` is an `ApplicationRunner` and re-creates the plans with NEW ids — so the tenant's `org_subscription.plan_id` matches nothing, `EntitlementResolver` falls through to `findByCode("FREE")`, and an ENTERPRISE tenant is silently *downgraded*, which is quieter than either symptom above; the bundle therefore carries the catalogue whole with its ids and `subscription.internal.PlanCatalogGuard` fails startup on the far side rather than serving it. And **`pg_dump` is not shelled out to**: it is a version-coupled client binary absent from the distroless image, only one of the eleven items is a faithful copy of a schema, six of them are transformations rather than copies, and the schema half comes from the migration set anyway — a dumped schema arrives carrying the SOURCE's `flyway_schema_history`. `BundleScriptWriter` emits the restorable form; an operator taking item 1 by hand with `pg_dump -n t_<hex>` is a perfectly good runbook step and is what `docs/runbooks/tenant-extraction.md` documents.

Item 8 is destructive, so the bundler has two modes: `REHEARSAL` reports the keys a cutover would revoke and revokes nothing, `CUTOVER` does it. The manifest records which — a bundle you cannot tell apart from a rehearsal is a bundle whose keys you do not know the state of.
...
Deliverables: the cache registry (GLOBAL/TENANT declaration, `getCache` fails on undeclared, TENANT keys prefixed in L1 and L2, throws when absent), `person-by-subject` and `org-by-external-id` declared GLOBAL, the hand-rolled prefix removed from `org-entitlements` (**`org-permissions` keeps its organization — §3.5.1, settled in Phase 5**), `SubscriptionService`'s `allEntries` evict fixed, `l1-max-size` raised. An axis declared on all 14 `@SchedulerLock` jobs, resumable cursors, re-derived `lockAtMostFor` and `RUN_DEADLINE` values. `platform.queue_signal` + the two-step claim for all three queues, with fairness.
...
Deliverables: the promotion runbook and its automation — freeze (HTTP `RESTRICT` window **and** queue/job pause), create schema, migrate, copy, verify row counts and checksums, flip placement, evict caches (`TenantPromotionCaches.evictAfterPlacementFlip`, §3.5.2 — one call, after the flip commits and before the freeze lifts), unfreeze. Reverse promotion, which evicts the same set for the same reason. The silo-count ceiling enforced in the promoter. The bounded-fan-out form of `pageForQueue`, `lockBreached`, `SoftDeletePurgeJob`, and the retention jobs over `tenant_pool` + silos. The `org-permissions` key question closed in writing (§3.5.1).

**Gate:** a real org promoted to `t_<hex>` and demoted back, with per-table row counts identical at both ends and a measured freeze window recorded in the runbook. The full suite passes with the test fixture running one org in a silo and the rest pooled. `SoftDeletePurgeJob` completes within its lease with a silo present. The promoter refuses at the ceiling. A permission set resolved before the flip is not served after it, and a revoked membership denies the very next request **inside a silo** (AGENTS §5.5, re-proved on the second schema). Every cache in `CacheRegistry` carries a promotion verdict, enforced by a test that fails on the eighth cache.
...
**Landed: the bundler and the ten database-side items.** `compliance.internal.TenantBundler` (+ `TenantBundlePlan`, `BundleDisposition`, `BundlePart`, `PersonProjector`, `BundleScriptWriter`), `apikeys.ApiKeyReminting` + `V59__api_key_prefix_reservation.sql` for item 8, `subscription.internal.PlanCatalogGuard` for item 4's far side, and `docs/runbooks/tenant-extraction.md`. Tested by `TenantBundleTest` (including a real restore: a second database built from nothing by both migration sequences, given the emitted script, ending with the tenant's subscription joining a `platform.plan` row — the property item 4 is about), `TenantBundlePlanTest`, `PersonProjectionGuardTest`, `PlanCatalogGuardTest`, and one ArchUnit rule keeping `ApiKeyHashing.mint` to the single caller that consults the reservations. **A defect the restore test caught and nothing else would have:** the platform half was emitted in catalogue order, and `external_organization_organization_id_fkey` is the last real foreign key into `organization` — alphabetical puts the child first, so every bundle would have aborted on its second platform statement and taken the whole transaction with it. The plan is now topologically ordered from `pg_constraint`.

**Landed: item 7, the object half.** `compliance.internal.TenantObjectExtractionService` (extract + restore over `OrgObjectPrefixes`), the three bulk-copy methods on `files.FileStorageProvider` (`list` / `open` / `write`, with `ObjectPage` and `StoredObject`), and `TenantObjectExtractionTest` against real SeaweedFS. The bundler acknowledges it rather than performing it — the object store is a different subsystem and a bundle that claimed to have copied it would be claiming something it never did — so a manifest carries the two prefixes and refuses `requireComplete()` until whoever ran the extractor reports what it took.

**Landed: the dump and the restore, actually performed.** `TenantSchemaSelfContainmentTest` places a real organization in a real `t_<32hex>` silo, takes `pg_dump -n t_<hex>` inside the origin container, builds a **second** Postgres with only the platform sequence, and replays the dump into it under `psql --set=ON_ERROR_STOP=1`. Three things about it are load-bearing. `ON_ERROR_STOP=1` **is the assertion** — measured, not assumed: without it psql exits 0 having skipped the failing statement, and the test would report a successful restore of half a tenant. The extracted deployment's `tenant_pool` is left **empty** (only `migratePlatform` runs there, never `run()`), which removes the second home an unqualified name could fall through to — so if the tenant can be served, it is being served out of the schema that came in the dump and nothing else. And the tier for every table is read from `docs/DATA_MODEL.md` rather than from the database, the third independent parser of those `**Tier:**` lines, because reading the tier back out of the schema the migrations built would make the test agree with the migrations by construction.
...
**Gate outcome: PARTIAL, and the honest split is between the database and the jar.**

*Demonstrated.* The restore itself, twice over and by two different routes — the physical dump above, and the bundler's own emitted script replayed into a database built from nothing by both migration sequences. In the restored schema: every row of every tenant-tier table arrived, every foreign key resolves inside the schema and none leaves it, none is `NOT VALID`, a member's permissions resolve through `membership → org_role → role_permission` and a revoked member's resolve to empty against a role that still holds a grant (**AGENTS §5.5, re-proved on the far side of an extraction**), a ticket opens against the restored `sla_policy`, the real `TenantTierTables` planner covers the whole tier and exports it from the restored schema alone, the infrastructure tables are empty, `external_identity` is empty *and authorization still works* — which is what keying on `person.id` rather than the token subject buys — and the org's subscription joins a `platform.plan` row, which is the property §6 item 4 is actually about.

*Not demonstrated: the jar has never been started against a restored database.* Both tests drive SQL against the far side. "The jar boots with the platform host unroutable" therefore remains unexercised, and so does everything that only happens at boot — `PlanCatalogGuard`, whose entire reason for existing is the far side, has never run there, and neither has `TenantRoutes`. "A member logs in" is proven as a permission-resolution walk, not as a login. "The nightly jobs run once" is proven for the one statement shape in them that crosses the tier (`SoftDeletePurgeJob`'s active-hold anti-join) and for no other job.

**The seam that gap was hiding, found at the Phase 6 review and now closed.** `platform.tenant_placement` was judged `WRITTEN_FRESH` — correctly; a lifted tenant has exactly one placement, its own — but *"written fresh" named no writer*. The bundler never emitted it and never reported it, so it appeared in no manifest and no script; the runbook never mentioned it; the only thing that wrote one was `TenantSchemaSelfContainmentTest`, by hand, in test code, which is precisely why every assertion in that test passed. `TenantRoutes.readHome` answers `tenant_pool` for an organization the registry does not name — the right answer for an unpromoted tenant, and on the far side §1's worst outcome, because the destination ran both migration sequences and therefore *has* a `tenant_pool`, empty. A restored tenant with no placement row serves as an organization with no members and no history and lands its first write in a schema its `org_id` disagrees with, silently, at every layer. Since `silo-per-org` became the default that is the shape of **every** extraction, not an edge case. `BundleScriptWriter` now emits the row inside the bundle's transaction, deriving `schema_name` from `current_schema()` so it tracks the one editable `search_path` line instead of being a second thing to edit and the first to forget; `schema_version` stays NULL, which V57's header defines as *not yet recorded, never behind*. `TenantBundleTest` asserts the restored deployment holds exactly one placement, `ACTIVE`, naming the schema the tenant's rows actually landed in — asked of the catalogue rather than spelled as a literal, so it stays true for the silo restore an operator actually performs. **Every other assertion in that class addressed the far side by name and would have passed against a deployment that could not address its own tenant at all.**

**Also unclosed by the gate, and the runbook now says so:** `platform.org_membership_index` is `REBUILT`, and the only thing that rebuilds it is a 04:50 cron. Until it runs, `GET /api/v1/me/organizations` — the one person-first read in the codebase — returns an empty list for every member of the extracted deployment. They are in the organization and cannot see that they are. That is a restore step, not a background detail.

**Two dispositions shipped recorded twice, differently. Both are now decided, once, and the duplication that hid it is gone.** `TenantBundlePlan` and `TenantSchemaSelfContainmentTest.EXTRACTION_VERDICTS` had agreed table for table on ten of twelve and contradicted each other on two. **`legal_hold`:** the plan said it stays and the bundler *refused* an org under a live hold ("release the hold, or do not extract this tenant"); the test said `TRAVELS` and asserted end to end that the restored purge was blocked by a hold that came across. **`api_usage_daily`:** the plan said it stays (one billing period, two ledgers, immediate divergence); the test said `TRAVELS` (billing continuity). Both shipped and nothing failed, because the test hand-carries its platform half instead of calling the bundler — the same structural reason the placement seam survived.

**The decisions.** `legal_hold` **stays**: a hold is *custody* rather than data, so no copied row makes it lawful to hand held material to a deployment somebody else runs, and the bundler continues to refuse an organization under a live one. What was genuinely wrong was the sentence it printed — telling an operator to release a litigation hold so a move may proceed is advice this code has no business giving — so the refusal now says *stop the extraction, not the hold, and escalate*, and the runbook says the same. `api_usage_daily` **stays**, and the deciding argument is a column rather than a preference: `exported` is `UsageExportJob`'s half of the double-billing guard, a carried row arrives with it false, and the far side would re-price days the platform has already invoiced — item 6's hazard exactly. An operator who needs one tenant's history takes it by hand and records that in the handover.

**And the duplication is structurally closed.** The judgement lives in `TenantBundlePlan` alone; `testsupport.ExtractionVerdicts` is the promotion-side mirror of the org-scoped subset, spelled in `BundleDisposition`'s own vocabulary, and `compliance.internal.ExtractionVerdictAgreementTest` fails the build if the mirror ever says anything the shipped plan does not — in either direction, including a constant added to one enum and not the other. The self-containment test now *derives* which tables it carries from that judgement instead of listing them, and asserts the stays-behind set arrived empty for the org (the origin seeds a live hold and a usage day precisely so those zeros are absences of rows that existed). Its far-side purge assertion was re-expressed rather than deleted: it now proves the hazard the refusal exists for — the hold did not travel, the aged membership *is* purgeable on the far side, and a hold written into the extracted deployment's own table stops it again. **And the other half is now closed too: the self-containment test's far side runs against the bundler's actual emitted output.** The ninety lines of hand-written `copyRows(...)` are gone; `TenantSchemaSelfContainmentTest` takes the bundle through `TenantBundler` → `BundleScriptWriter` → `psql`, reached through `compliance.internal.TenantBundleArtifact` — a class in the bundler's own package, in the TEST source root, whose entire public surface is `String`, `List` and `UUID`, so nothing in `src/main` was widened to make a test reachable and the self-containment test still cannot name a compliance type. Route 1 (`pg_dump -n t_<hex>` by hand) is kept beside it because the two fail differently: only the physical dump can say a tenant schema is restorable at all, and only the bundle can say the shipped bundler carries what it claims. The restore is into a `t_<32hex>` silo with the script's one editable `search_path` line actually edited — which is what makes `tenant_placement.schema_name`'s derivation from `current_schema()` an assertion rather than an argument, and it is the claim `TenantBundleTest` structurally cannot make, since it restores into `tenant_pool` where the default and the truth are the same string.

**The gate was checked against being uncheckable, which is the part worth recording.** Four separate defects were injected into the shipped bundler and the gate went red on each, every one naming a different failure: `organization` judged `STAYS_ON_THE_PLATFORM` aborts the restore itself on `external_organization_organization_id_fkey` (caught by `ON_ERROR_STOP=1`, which is why that flag is the assertion); a `BundleScriptWriter.epilogue` that stops emitting the placement row fails on `WRITTEN_FRESH` holding zero rows — the exact seam this section says every prior assertion would have passed over; `plan` dropped from the catalogue snapshot is refused at plan time by `requireTheCatalogueIsWorthShipping` before a bundle exists; and a tenant-row copy that silently skips `role_permission` fails the far-side row counts at 3 → 0. Before this change all four were green.

**Still genuinely open, and it is not a test problem:** nothing in `src/main` calls `TenantBundler`. `docs/runbooks/tenant-extraction.md` §"Taking it" is written as though an operator entry point exists, and it does not — the bundler has one caller and it is the test suite. Deciding what that entry point is (a `main`, an admin surface, neither) is a decision about **authorization** before it is a decision about plumbing, because an extraction reads every secret the tenant owns, and it is deliberately not being made here.

**One boot check was doing the thing this document forbids, and is now scoped.** `PlanCatalogGuard` quoted `MappedSchemaValidator`'s rule — *silos are a fleet-wide question that a single pod cannot answer* — and then fanned its check out over every ACTIVE silo and threw on the result from an `ApplicationReadyEvent` listener, which `SpringApplication.run` rethrows. Two failures followed. A stale ACTIVE placement row naming a schema that no longer exists (a state V57's header and `TenantMigrationRunner.discoverFleet` both name as legitimate, and which `TenantRoutes.readHome` hands to `search_path` with no existence probe) raised `relation "org_subscription" does not exist` and failed **every pod's** boot; and one organization out of five thousand with a dangling `plan_id` did the same, converting a one-tenant silent downgrade into a total outage. The judgement is right on the extracted single-tenant deployment §6 item 4 is about and was never right on the pooled fleet. Now: `tenant_pool` still refuses, a silo refuses **only when `platform.tenant_placement` holds exactly one tenant** — which is the extracted deployment — and on a fleet a dangling silo plan is a WARN naming the tenant, the plans and the query an operator runs. Every per-silo probe is wrapped, so a schema that has gone missing is reported and never thrown out of the ready listener.
...
**Landed: the gate, run against two real databases and measured.** `TenantDatabaseCutoverTest` drives the shipped `TenantCutover` — real logical replication, not hand-wired streams — end to end: a >5,000-row org (memberships, roles, tickets with messages, a subscription joining `platform.plan`) moved to a second Postgres with **writes held 4.3–4.4 s** (2 s settle + drain + verify + one route-TTL hold at the suite's PT2S; the size-scaled copy ran online, ~2 s of sync), decommissioned on the source, then moved BACK by the same machinery with the roles swapped — 4.3 s again — because "reversible" unproven is a hope. Counts **and content checksums** are recomputed by the test's own SQL over direct connections to each container, never taken from the machinery's report (whose numbers are asserted against them); a keyset cursor minted while the tenant lived on primary pages correctly after **two** database moves; the pooled neighbour's rows, placement and service are untouched; and §5.5 across the split is the very next request: revoke post-flip, the next request is 403, the unrevoked control still serves, and the revocation **survives the return trip**. `RemoteTenantPlatformOutageTest` is the outage half, against a genuinely severed primary (`ALLOW_CONNECTIONS false` + `pg_terminate_backend` — reversible on the same ports, and the failure travels the whole driver→Hikari→translation chain): the remote-served silo answers whole requests 200 on held-over identity inside the ceiling; a mid-outage revocation refuses the very next request (503 + `Retry-After`, nothing cached — never the remembered "yes"); past the ceiling identity denies with the same shape and recovery follows reconnection with no restart; a route the process no longer remembers fails closed naming the refusal rather than guessing a pool.

**What the gate caught, which is why it exists.** Four defects in shipped code, each found only by executing the topology: (1) `PermissionResolver`'s org-status read ran on the caller's tenant connection — correct for five phases, dead on a remote whose minimal `platform` schema is §5.1's tripwire; it is now its own platform-axis span, and when the primary is unreachable it **refuses** (the sanctioned 503, nothing cached) rather than serving authorization off a standing nobody could confirm. (2) The stale classifier missed SQLState 57P01/57P02/57P03 — the shape a failover or terminated backend actually delivers — so a real outage would have rethrown as a "bug" instead of degrading to the holdover. (3) The 503 renderer itself read `platform.translation` through a cold cache mid-outage, turning the outage's own curated answer into an unenveloped 500; `ErrorDetailLocalizer` now serves the author's default. (4) The first test context on a private database poisoned the process-wide `TenantRoutes` slot for every cached context (`TenantRoutesTestInstalls` is the reinstall discipline, and the three-class sequence proving it is in the suite).

**Falsifiability, performed rather than claimed.** With `copy_data` forced false AND the machinery's own verification blinded, the gate went red on the test's independent counts — expected `{audit_log=5000, membership=2, …}`, got all zeros — a cutover that "succeeded" while moving nothing, caught by nothing the code under test supplied. Also recorded honestly: a skipped post-flip eviction+hold is NOT observable at the suite's PT2S route TTL (the reverse-stream tail outlasts the memo), so that mechanism is pinned by the `freezeHeld ≥ route-ttl` bound and `TenantRoutes`' own tests, not by the gate's behavioural asserts.

**Specified here, deliberately not proved here:** the HTTP adapters and everything Phase 8 (the port contracts are recorded in the ports' javadoc); multi-pod TTL healing (one JVM cannot re-read a route it did not forget); `PlanCatalog`'s throw-at-ceiling on a remote tenant's quota path end to end (unit-pinned); and the mid-cutover migration refusal and its REFRESH repair, which the cutover builders' own tests carry.
...
The original answer was: ~0.5–0.66 ms of planning per UNION branch (not amortized) and 2.004 lock entries per branch against ~6,400 cluster-wide slots; **200 silos per database, enforced in the promoter**; at 200 that is ~130 ms of planning and ~403 locks per fan-out transaction. It closed by saying **this is the one number in the document I would most want re-measured before it is load-bearing** — the experiment being to replay the UNION benchmark at N = 50 / 100 / 200 against real `tenant_pool` + silo data, not synthetic schemas.

**Phase 5 made it load-bearing and the experiment has now been run. RESOLVED: 200 stays, and every number in the paragraph above it is wrong.** Planning per branch is overstated by 12–15×, locks per branch are understated by 3.5×, the lock capacity both are measured against is 2.3× larger than assumed — and the cost that actually binds at 200 belongs to a query shape this document never modelled, because Phase 5 shipped a per-home merge rather than the UNION. Raw data, method and reproduction: [`docs/reviews/2026-08-08-adr-0010-q1-silo-ceiling-remeasurement.md`](../reviews/2026-08-08-adr-0010-q1-silo-ceiling-remeasurement.md).

**Method, so the numbers can be argued with.** Postgres 18.4 with `max_connections = 100` and `max_locks_per_transaction = 64` — the pair §1 turns into "~6,400 slots". The fleet was built by the real `TenantMigrationRunner` off the classpath: the 44-file platform sequence into `platform`, the 33-file tenant sequence into `tenant_pool` and into **200 real `t_<32hex>` silos** with `ACTIVE` placement rows (8 workers, 22.8 s, zero failures). Real rows, because empty schemas do not reproduce the planning cost the estimate was about: **320,000 `ticket`** (120,000 in `tenant_pool` spread over 4,800 orgs, 1,000 per silo), 960,000 `ticket_message`, 320,000 `audit_log`, 5% soft-deleted, `ANALYZE`d per schema, 716 MB. The query is `TicketRepository.pageForQueue`'s shape, one `UNION ALL` branch per home, each branch resolving to an `Index Only Scan using idx_ticket_status_recent`. Caveat that limits every absolute below: this ran on a laptop under a VM, not the box the original numbers came from, so what is load-bearing is each **ratio between two measurements on the same box**, never an absolute against the ADR's absolutes.

**Finding 1 — planning per branch was a COLD-BACKEND number, and it amortizes twice.**

| branches | cold backend (first execution on a new connection) | warm backend (same connection, run ≥ 2) | server-side prepared (run ≥ 7) |
|---|---|---|---|
| 50 | 16.6–27.9 ms → **0.33–0.56 ms/branch** | 1.19–1.82 ms → **0.024–0.036** | 0.026 ms |
| 100 | 29.7–38.7 ms → **0.30–0.39 ms/branch** | 2.35–2.83 ms → **0.024–0.028** | 0.051–0.058 ms |
| 200 | 58.7–82.1 ms → **0.29–0.41 ms/branch** | 6.95–8.86 ms → **0.035–0.044** | 0.086–0.098 ms |

The 0.5–0.66 ms/branch figure reproduces only on the first execution on a backend that has never touched those relations. Steady state on a pooled connection is **12–15× lower**; under a server-side prepared statement (which pgjdbc creates at its default `prepareThreshold = 5`) it is **80–100× lower**. Ten consecutive executions on one connection at 200 branches: run 1 = 57.4 ms, runs 2–10 = 6.95–8.42 ms. **"Planning does not amortize — warm re-runs pay it again" is false.** So is the reason given for it: the 200-branch query text is 67,627 bytes (338 bytes per branch, so 5,000 branches is ~1.69 MB, not the ~730 KB §5.1 states) against pgjdbc's 5 MiB / 256-query statement cache — **the cache is not defeated at the ceiling**, only a long way past it.

**Finding 2 — locks per branch are 7.000, not 2.004, and the reason is exactly what §8 Q1 was afraid of.** Measured 9 / 72 / 352 / 702 / **1,402** total entries at 1 / 10 / 50 / 100 / 200 branches — 7.000 per branch at every count, all `relation`/`AccessShareLock` plus one `virtualxid`. The planner locks a table **and every one of its indexes**, and the real `ticket` has six (`ticket_pkey`, `idx_ticket_org`, `idx_ticket_sla`, `idx_ticket_deleted`, `idx_ticket_status_recent`, `idx_ticket_recent`). 1 + 6 = 7. **2.004 is what a table with one index yields: the original measurement was not taken against this schema** — which is precisely why §8 Q1 asked for real silo data rather than synthetic schemas. It also means this is not a constant of the design but a function of the fan-out's table: adding an index to `ticket` raises it at every branch at once.

**Finding 3 — `max_locks_per_transaction × max_connections` is not a per-transaction cap.** At 10 × 100 (nominal 1,000) a single transaction held 1,409 relation locks without error. Probed directly, one transaction touching table after table until it died: **14,751 locks at the 64 × 100 default (2.3× nominal)**, ~3,150 at 10 × 100 (3.1× nominal). Failure mode confirmed exactly as described — `ERROR: out of shared memory`, SQLSTATE 53200, whole transaction lost. So the real UNION wall is ≈ 14,751 ÷ 7.0 ≈ **2,100 branches**, not ~3,200 and not the ~914 a naive 6,400 ÷ 7.0 gives. **Do not design against 2,100**: it is a single-transaction figure on an idle cluster and the lock table is shared. The honest number is the fraction one fan-out takes — at 200 branches, 1,402 entries = **22% of the nominal 6,400, 9.5% of the measured ceiling**. (The same capacity error propagates to §5.12: `drop schema … cascade` fits **30** schemas per transaction, not 15 — 487 locks per schema, failing on the 31st. §5.12's *advice*, one schema per transaction, survives and is now known to be conservative by 2×.)

**Finding 4 — the ceiling's real constraint is the fan-out shape that actually shipped, and the ADR does not model it.** `TicketFanOut` does not build the UNION: Phase 5 shipped the N-way merge, one keyset query per home in its own transaction. So the whole planning-versus-locks derivation describes a statement this codebase never issues.

| homes | UNION, warm, server-side (plan + exec) | **shipped per-home merge, total** | per home | max locks in any one transaction |
|---|---|---|---|---|
| 50 | ~15 ms | **76.9–103.2 ms** | 1.54–2.06 ms | **9** |
| 100 | ~20 ms | **129.6–145.8 ms** | 1.30–1.46 ms | **9** |
| 200 | ~34 ms | **258.3–278.6 ms** | 1.29–1.39 ms | **9** |

`TenantFanOut`'s javadoc is right about locks — 9 entries at any instant whatever the fleet size, so the shared-memory ceiling is genuinely never approached. It is wrong about cost: the per-home shape is **~7× slower per page** than the UNION it replaced, flat at 1.29–1.39 ms per home from 100 upward, and **279 ms per operator queue page at 200 silos** — 2× §5.1's own "~130 ms, acceptable for an operator listing" budget, on an interactive surface, with `SlaEscalationJob` paying a fan-out of the same shape once a minute.

**Finding 5 — the footprint and catalog claims are ~2.6× overstated, because they are the UNIFORM numbers.** §1's "55 tables + 184 indexes = 239 relations, ~3.2 MB" counts every table in every schema — the design §1 rejected. A real silo built by the real runner holds the tenant tier only: **28 tables + 98 indexes = 126 relations** (plus 13 toast tables and 13 toast indexes, so 152.8 `pg_class` rows per silo measured as a delta), and **912 KB** of empty relation files, not 3.2 MB. Catalog grows **~542 KB per silo** (`pg_class` +152.8, `pg_attribute` +776.9, `pg_index` +111.6, `pg_depend` +623, `pg_constraint` +233 rows each). Scaled: **at 200 silos, 178 MB of empty files and 115 MB of catalog** (§5.10 said ~48,000 relations and ~650 MB); **at 5,000, ~4.35 GB of empty files, ~765,000 `pg_class` rows, ~3.89M `pg_attribute` rows, ~2.65 GB of catalog** (§1 said ~16 GB, ~1.2M, ~6M, 5–6 GB). **§1's rejection of uniform schema-per-tenant stands** — 4.35 GB of empty files and 765k catalog rows is still not shippable into this database — but it stands at roughly 40% of the magnitude it is argued at, and anyone re-opening that decision must re-open it against these numbers rather than the ones in §1.

**Finding 6 — JDBC metadata at the ceiling is 287–527 ms, not 9 s.** Unfiltered `getColumns` across 201 tenant schemas: **65,208 rows, 526.5 / 371.9 / 323.5 / 287.4 ms** over four fresh connections; filtered to one schema, 312 rows / 12.7 ms. 312 rows per tenant schema means ~1.56M rows at 5,000 silos (§1's ~3.1M is the uniform count again) and ~7–12 s at the measured per-row cost, which does bracket §1's ~9 s. At the ceiling we actually allow it is half a second, once, at boot — and moot regardless, since `ddl-auto` is already `none` and `MappedSchemaValidator` runs the check on two pinned axes after `ApplicationReadyEvent` and issues no unfiltered metadata call at all. §4.4's decision was right; its urgency was overstated by an order of magnitude.

**The pattern, because it is the most transferable thing here.** Every number this run contradicts is contradicted the same way: **a cold-cache, first-execution measurement recorded as a steady-state cost** — UNION planning (0.5 ms/branch cold → 0.035 warm), `pg_dump -n` of one schema (§5.11's 2.9 s reproduces cold; warm it is 0.29 s), JDBC metadata (527 ms first call → 287 ms fourth). The one number that moved the *other* way — locks per branch, 2.004 → 7.000 — is the one that is not a timing at all but a structural count, and it moved because it was measured against something that was not this schema.

**THE ANSWER. Keep `app.tenancy.promotion.max-silos` at 200, and replace the derivation.** The number survives; its reasons do not. Neither cost the ADR named binds: planning amortizes to 7–9 ms at 200 branches, and the shipped fan-out issues no UNION at all and holds nine locks whatever the fleet size. What binds is `TicketFanOut`'s per-home merge at 1.33 ms per home — **279 ms per operator queue page at 200 silos, linear in home count, on an interactive surface**. That is already 2× the budget §5.1 wrote down, and it is reached at exactly 200, so the ceiling is right by arithmetic that no longer resembles the arithmetic that chose it. Two consequences follow and both are load-bearing:

- **Q2's `platform.ticket_index` trigger at 50 silos is now the item that decides whether the ceiling can ever rise**, not a note for later — at 50 silos the queue page already costs 77–103 ms. Once the queue and `lockBreached` read a platform-side index, the per-home latency that binds today stops applying and the ceiling should be re-derived from the footprint half (§1), which at 542 KB of catalog and 912 KB of files per silo would permit a far larger fleet.
  > *Half done, and the half that is missing is the half that still binds. The QUEUE reads the index (V61, §8 Q2), so the interactive surface no longer pays per home. `lockBreached` does not and will not without a measurement (§5.2's note), so `SlaEscalationJob`'s once-a-minute fan-out is now the only per-home cost at the ceiling. **The 200 stays until that is re-measured**: it was chosen by arithmetic over the queue page, and that arithmetic no longer applies to the thing it was derived from. The experiment is to time one `escalateBreaches` pass at N = 50 / 100 / 200 real silos against `RUN_DEADLINE`'s four minutes, and then re-derive the ceiling from the footprint half.*
- **`TenantPromotionProperties.DEFAULT_MAX_SILOS`' javadoc and `TenantPromoter.refuseAtTheCeiling`'s refusal message still quote the superseded derivation verbatim** — "0.5–0.66 ms per branch", "2.004 entries per branch", "50 → ~102, 200 → ~402" — to an operator who has just been refused a promotion, which is the only place anyone reads this reasoning. Correcting them is production code and was deliberately out of scope for the measurement pass; it is the immediate follow-up.
...
**RESOLVED for `ticket_index`: the trigger fired, and it fired for a reason this question did not anticipate. Built as V61 (`platform.ticket_index`), with its reconciler in the same change-set.** Two things moved under the default. Commit 0822943 made `silo-per-org` the DEFAULT placement, so "50 silos" stopped being a promotion milestone and became *fifty organizations* — the home count is now the tenant count. And Q1's re-measurement replaced the cost model the trigger was set against: the shipped per-home merge is **1.29–1.39 ms per home, flat from 100 upward**, so the interactive operator page is 279 ms at 200 homes and grows linearly for as long as the business does. Neither of the costs §5.1 originally argued (UNION planning, lock slots) was ever going to fire; this one already had.

**What was built, and the three decisions inside it that were NOT in the sketch above.**

- **The column set is the wire contract's, not §5.1's nine.** `platform.ticket_index(ticket_id, org_id, status, priority, created_at, resolution_due_at, escalated, subject, assignee_person_id)` predates the resource `GET /api/v1/admin/tickets` actually returns, and is missing `opener_person_id`, `category` and `first_response_at`. A queue rendering three attributes as null is a behaviour change no caller could see coming, so the table carries what the operator's page orders, filters and renders — and deliberately nothing else. `version`, `created_by`/`updated_by` and `deleted_at` are absent: the row's existence is the liveness statement, and there is nothing here to lock against or to authorize with.
- **It is maintained from the tenant-tier writes, not from an event listener.** The sketch said "fed by the existing event-listener + `EventInbox` mechanism, the pattern `search_document` already proves". `support` publishes exactly one event (`TicketEscalated`), so that route would have meant inventing four more events to carry a projection. `SupportService` is already the single funnel both protocol surfaces pass through, so the upsert sits there, inside the ticket's own transaction, through `CrossDatabaseWrites` — which is the same connection and the same transaction whenever the tenant is co-located with primary, and a separate transaction on primary when ADR 0011 has moved it. **Which means the pair is NOT atomic for a remote tenant, and the ordering was chosen rather than inherited:** the index row may outlive its ticket (a ghost in the queue that 404s on click-through), never the reverse (a customer's ticket the desk cannot see, which is the Phase 5 failure this module already paid for once).
- **The reconciler is the delete path, not a backstop for it.** Nothing in the application soft-deletes a ticket, so every disappearance is raw SQL that cannot know the projection exists — `SoftDeletePurgeJob`'s hard delete past retention, `SoftDeleteRecovery.restore`, a schema dropped whole. That is `sweepSearchResidue`'s situation exactly. `TicketIndexReconciler` fans out over homes with `TenantHomeSweep` (PT30M lease against a 25-minute run deadline, re-derived for this fan-out) and runs two **independent, individually resumable** arms: refresh every live ticket, and sweep index rows the home no longer has. They are independent because the obvious single-pass shape — collect every live id, delete everything not in the set — is one deadline away from deleting the tail of a tenant's queue. The sweep therefore never reasons from absence: it takes a page of index rows and asks the tenant which of *those* ids are live. Neither arm can be one SQL statement, because across two databases `insert into platform.ticket_index select … from ticket` is not a slow statement, it is not a statement.

**`audit_index` is unchanged: still never**, and nothing in Q1's re-measurement bears on it — `/api/v1/audit` without `?org=` returns platform-scope rows and there is no cursor that survives merging several tenants' trails.

**What did NOT move to the index, and why.** `SlaEscalationJob` still fans out. §5.2 says it should "read `platform.ticket_index` and dive per-tenant only to write the escalation", and the reason that is a listing's answer and not a sweep's is the row lock: `lockBreached` is a `PESSIMISTIC_WRITE`/`SKIP LOCKED` select on the tenant's own table, which is what stops two instances escalating one ticket twice, and a projection cannot lock a row in another schema — let alone in another database. Reading candidates from the index still means re-reading and re-locking each one in its home, so it only pays once the breach count is far below the home count. That is a measurement nobody has taken, and the deadline on that job is what bounds it until somebody does.
...
1. ~~**Are `document.storage_key` values org-namespaced in SeaweedFS?**~~ **SETTLED — yes, and the experiment found something worse on the other side of the same boundary. Kept numbered so items 2–4 keep their names.**

   **The answer: every org-scoped key is org-namespaced, and the `files` module mints nothing.** `FileStorageProvider` takes a caller-supplied key and `S3StorageProvider` never constructs one — there was no minter in `files` to read. Exactly four classes write bytes to the object store (`put` / `putLarge` / `presignPut`), and between them they mint five key forms, which is all of them:

   | minter | writer class | key | owner |
   |---|---|---|---|
   | `OrgDocumentController.store(file, "o/" + orgId, …)` | `OrgDocumentController` | `doc/o/<orgId>/<uuid>/<safeName>` | org |
   | `ExchangeService.submitImport` / `ArtifactStore.artifactKey` | `ArtifactStore` | `exch/o/<orgId>/<uuid or jobId>/<name>` | org |
   | `PersonalDocumentController.upload` → the same `store` step | `OrgDocumentController` | `doc/u/<personId>/<uuid>/<safeName>` | person |
   | `FileController.newKey` | `FileController` | `u/<personId>/<uuid>/<safeName>` | person |
   | `ProfileService.changeAvatar` | `ProfileService` | `avatar/p/<personId>/<uuid>` | person |

   Two of those person namespaces this document had never named: `u/<personId>/` (raw files, **no `document` row at all** — those bytes are addressed by key alone and are therefore invisible to retention, `legal_hold` and the extraction bundle) and `avatar/p/<personId>/` (V28's spelling, one object per person keyed from `person_profile.avatar_key`). Both are person-scoped, so both correctly stay behind. §6 bundle item 7's two prefixes are complete.

   **The premise this question rested on has expired, which makes the prefix load-bearing rather than merely tidy.** "`uq_document_storage_key` is globally unique so keys do not collide today" stopped being true at Phase 2: the index now exists once per schema (`platform/V23` and `tenant/V23` both build it), so two tenants *can* hold the same key and nothing in Postgres says so. The `o/<orgId>/` segment is the only separator left — and the `o/` matters, not just the id, because an org id and a person id are both UUIDs, so `doc/<uuid>/` cannot be told from the personal namespace it might be.

   **Enforced, not asserted.** `NewDocument`'s compact constructor refuses a registration whose `orgId` and `storageKey` disagree — in both directions: an org document must carry `o/<orgId>/`, and a personal one must **not** sit under any org's prefix (extraction copies a prefix whole, so a personal row parked there would leave with a tenant regardless of its `org_id`). That record is the one door every `document` row goes through, so the check is complete for the org-scoped surface. `StorageKeyNamespaceTest` pins the rule and enumerates the four writers from the compiled classes, so a fifth fails the build until its author says which owner its bytes belong to; `DocumentExtractionBoundaryTest` drives the live minters over HTTP.

   **Amended when the extractor was actually built: naming the organization is necessary and NOT sufficient, and the rule now checks the whole prefix.** The original guard asked whether the key contained `/o/<orgId>/` anywhere. A bucket is a flat key space with no such query — the bundle **lists** `doc/o/<orgId>/` and `exch/o/<orgId>/` — so `report/o/<orgId>/…` satisfies every reading of "the key names its org" and is still a byte no extraction would ever visit. The declared roots therefore live in one place, `shared.document.OrgObjectPrefixes`, which the registration guard, the extraction and the restore all read; adding a namespace is one line there and it is extracted, refused-when-foreign and restorable the same day. The second, independent gate is the one that catches a key written around the guard entirely (a legacy row, a migration): `TenantObjectExtractionService` re-asks the question of the tenant's own recorded keys at extraction time and refuses rather than under-copying. Both are needed and they fail at different moments — one when the code is written, the other when the tenant is moved.

   **What the experiment actually found — a real bug, now fixed.** The keys were right; the **rows** were not. `document` is a split table whose entity names no schema, and `CurrentUserFilter` pins the caller's organization whenever their token names exactly one — *whatever the route*. `POST /api/v1/documents` is not under `/api/v1/orgs/{orgId}/`, but a single-org member (every one of the 193,940 seeded people) still arrived on their org's axis, so their **personal** document was written into that tenant's schema with a null `org_id`: the row-disagrees-with-its-schema failure §1 calls the worst this design can produce. Consequences, all silent — `pg_dump -n t_<hex>` would have carried a human's private documents out with an organization that never owned them (and without their bytes, whose `doc/u/<personId>/` key is in neither bundle prefix); promotion's `where org_id = ?` copy would have stranded them in `tenant_pool`; a member joining a second organization resolves to *no* organization and would have lost sight of them entirely; and `AuditLogImpl` routes on the row's own org, so `document.registered` went to `platform.audit_log` while the document went to the tenant's copy. **Fix:** every `PersonalDocumentController` handler pins `TenantContext.runAsPlatform` / `callAsPlatform` around its service call — the same shape and the same reason as `AdminMaintenanceController.schedule`, which is this bug's mirror image. `Document`'s javadoc asserted the opposite ("names no organization, so the request stays on the platform axis", citing a route that does not exist) and has been corrected.

   **And the half this document never answered: personal documents belong to no tenant, so nothing happens to them at extraction.** `document.org_id IS NULL` is the platform copy, the human keeps it, and the extracted deployment's `/api/v1/documents` is empty for its members — correctly, since a person in orgs X, Y and Z does not fragment when X leaves (§2.2). That is a visible behaviour change for a user of an extracted tenant and belongs in the runbook, not in a bug report. The same holds for `u/<personId>/` files and `avatar/p/<personId>/` avatars, whose rows (`person_profile`) are platform-tier already.

   **The multi-org person makes the answer forced rather than merely defensible.** A human in orgs X, Y and Z has ONE personal document store, addressed by their `person.id` and by nothing else. Sending it with X is a leak (Y and Z's operators are not party to it, and the human never consented to it leaving); withholding it from a single-org member's extraction would be a special case that the two-org member's cannot have, so the rule would hold only for the population that happens to be simple today — which is 193,940 of 193,940 people in the seed and therefore exactly the shape that stays untested until it breaks. Copying it to X *and* keeping it produces two divergent copies of a human's private files with no reconciliation and no erasure path, which is worse than either. One store, one owner, stays with the owner: the same answer §2.2 gives for `person` itself, and the reason the person namespaces are not in item 7's prefix list.
