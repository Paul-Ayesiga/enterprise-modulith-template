...
| A PLATFORM-tier table is **always** named `platform.<table>` — in `@Table(schema = "platform")`, in `@SQLDelete`/`@Query`, in every `JdbcTemplate` string. A TENANT-tier one is **never** qualified, so `search_path` places it (ADR 0010 §2; the tier is the **`Tier:`** line in `docs/DATA_MODEL.md`) | `PlatformSchemaQualificationTest` — the mapping rule over `@Table`/`@CollectionTable`, and the source scan over every SQL literal in `src/main/java` | ArchUnit failure naming the table and the file:line |
| A table name built at runtime resolves its schema through `shared.persistence.MappedTables` (off the entity mapping), never from a second list of which tables are the platform's | `SoftDeletePurgeJob.purgeTable`, `SoftDeleteRecovery.tableOf` are the reference implementations; a hand-kept list is invisible to the rule above | review rejection |
| Every Valkey key goes through `shared.deployment.DeploymentIdentity.valkeyKey`, and the object-store bucket is `DeploymentBucket`'s — never `StorageProperties.bucket()` (ADR 0010 §6 hop 2→3: two deployments share infrastructure, and a shared namespace **answers with the other installation's data**) | `DeploymentNamespaceTest` — the enumeration of every Valkey-touching class from the compiled classes, and the single permitted reader of the configured bucket | test failure naming the class |
| At the edge, a **route-scoped** resilience filter must never answer for a request re-targeted onto a tenant's own deployment (ADR 0010 §7 Phase 8). Those filters are named and calibrated per ROUTE, and the retarget deliberately keeps the route id, so one dead tenant deployment trips the instrument every other tenant on that path shares — and a fallback swallows the per-tenant refusal into a bare 503 | `TenantAwareCircuitBreaker`; `TenantUpstreamTest.aDeadTenantDeploymentKeepsItsRefusalAndNeverOpensTheSharedRoutesCircuit` (five failures on a `circuit-breaker: true` route, then a pooled tenant must still get 200) | test failure |
...
| Shared infrastructure | `shared.deployment.DeploymentIdentity` (`app.deployment.id`) | one name derives the Valkey namespace **and** the object bucket. `platform` is reserved and keeps today's keys byte-for-byte; every other value namespaces both. Never add a second, independently configured prefix — three knobs are three chances to forget one |
...
- **Expand (ship N):** additive only — new tables, new *nullable* columns (or `not null` with a
  ...
  unchanged against the expanded schema. Everything through V48 is expand-shaped; keep it that way.
